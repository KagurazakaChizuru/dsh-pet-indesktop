...
# ------------------------------- show_bubble 的返回值：有没有真的显示上去
# 歌词控制器每拍重发同一句，并靠这个返回值决定"要不要记账"（见
# pet/music_lyric_controller._show）。返回 None/True 会被当成"显示成功"，
# 所以被丢弃的每条路径都必须明确返回 False。


def test_show_bubble_returns_true_when_shown(win):
    assert win.show_bubble("一句歌词", duration_ms=3000, subtitle="歌名",
                           title_first=True) is True
    assert win._speech_bubble.shown[-1]["text"] == "一句歌词"


def test_show_bubble_returns_false_while_alert_is_showing(win):
    """提醒展示期间普通气泡让路：返回 False，且绝不覆盖提醒。"""
    win.show_alert("审批一", sticky=True)
    assert win.show_bubble("一句歌词", duration_ms=3000) is False
    assert win._speech_bubble.shown[-1]["text"] == "审批一"


def test_show_bubble_returns_false_when_window_hidden(win):
    win.hide()
    assert win.show_bubble("一句歌词", duration_ms=3000) is False


def test_show_bubble_returns_false_when_suppressed(win):
...
def test_sticky_and_button_bubbles_still_return_true(win):
    """sticky / 带按钮的气泡本来就要占用气泡位：返回 True，不受提醒让路影响。"""
    win.show_alert("审批一", sticky=True)
