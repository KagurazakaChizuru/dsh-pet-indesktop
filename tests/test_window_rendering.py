...
def test_self_talk_styles_builtin_text_with_custom_phrases():
    """内置自言自语必须过 dialogue_mode 风格路由：custom 模式下内置文案
    应被 dialogue_phrases 的 thinking 台词替换（回归：expression_style_text
    实现存在但从未接线，custom 设置对内置自言自语零效果）。"""
    _qapp()
    from pet.config import DEFAULT_SELF_TALK_TEXTS

    class Cfg:
        def get(self, key, default=None):
            return {
                "dialogue_mode": "custom",
                "dialogue_phrases": {"thinking": ["自定义思考句甲"]},
            }.get(key, default)

    pet = _FakeBubblePet()
    pet.cfg = Cfg()
    pet._self_talk_texts = [DEFAULT_SELF_TALK_TEXTS[0]]
    pet._self_talk_images = []
    pet._self_talk_duration_seconds = 5.0
    pet._show_self_talk_text = lambda text: window_mod.PetWindow._show_self_talk_text(pet, text)

    assert window_mod.PetWindow._show_random_self_talk(pet) is not False
    assert pet._speech_bubble.shown_text == ["自定义思考句甲"]


def test_self_talk_keeps_user_text_unstyled_in_custom_mode():
    """非内置文案（用户自己的自言自语条目）不得被风格路由改写。"""
    _qapp()
    ...
    class Cfg:
        def get(self, key, default=None):
            return {
                "dialogue_mode": "custom",
                "dialogue_phrases": {"thinking": ["自定义思考句乙"]},
            }.get(key, default)

    pet = _FakeBubblePet()
    pet.cfg = Cfg()
    pet._self_talk_texts = ["用户自己写的话"]
    pet._self_talk_images = []
    pet._self_talk_duration_seconds = 5.0
    pet._show_self_talk_text = lambda text: window_mod.PetWindow._show_self_talk_text(pet, text)

    assert window_mod.PetWindow._show_random_self_talk(pet) is not False
    assert pet._speech_bubble.shown_text == ["用户自己写的话"]
