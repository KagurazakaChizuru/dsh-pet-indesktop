...
def test_play_marks_self_speaking_and_terminal_status_clears(monkeypatch):
    """起播登记「桌宠在用自己的通道播报」，媒体终止态清除。

    否则音乐峰值检测会把自己的报时/节日语音当成音乐，触发唱歌动画。
    """
    from pet import music_detect

    _stub_qt_qurl(monkeypatch)

    svc = _make_svc({"volume": 100})
    svc._player = _PlayingStub()
    svc._audio_out = _OutStub()
    svc._terminal_statuses = ("END",)
    svc._pending_speech = None
    svc._stopped = False

    music_detect.clear_self_speaking()  # 自我登记是模块级状态：用例自带清零
    try:
        assert svc._play_only("cache.mp3") is True
        assert music_detect.is_self_speaking() is True

        svc._on_media_status("还在播")  # 非终止态：不清
        assert music_detect.is_self_speaking() is True

        svc._on_media_status("END")
        assert music_detect.is_self_speaking() is False
    finally:
        music_detect.clear_self_speaking()


def test_bubble_play_marks_self_speaking(monkeypatch):
    """带气泡的报时同样要登记（两条播放路径都不能漏）。"""
    from pet import music_detect
    ...
    svc._player = _PlayingStub()
    svc._audio_out = _OutStub()
    monkeypatch.setattr(svc, "_bubble", lambda text: None)

    music_detect.clear_self_speaking()  # 同上：不依赖别的用例的遗留状态
    try:
        svc._play_and_bubble("cache.mp3", "现在是下午三点整。")
        assert music_detect.is_self_speaking() is True
    finally:
        music_detect.clear_self_speaking()


def test_stop_clears_self_speaking(monkeypatch):
    """关掉报时/退出后不得残留登记（否则唱歌检测被永久屏蔽）。"""
    from pet import music_detect

    svc = _make_svc()
    svc._timer = _TimerStub()
    svc._pending_speech = None
