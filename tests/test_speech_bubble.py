...
# ------------------------------------- 歌词续期（同内容重发）与锁宽
# 背景：歌词控制器每拍（500ms）把同一句歌词重发一次。原实现每次 show_text 都
# `_reset_paging()` + 从第一页重排，页码计时器（≥2500ms）永远等不到 ⇒ 多页文本
# 永远只画第一页（实机表现："歌词只显示一半就剩歌名"）。


def test_same_content_renewal_keeps_paging_state(monkeypatch):
    """同内容续期：只续隐藏计时器，不重置翻页、不重建页面。"""
    _get_app()
    monkeypatch.setattr(speech_bubble_module, "PAGE_FADE_OUT_MS", 1)
    monkeypatch.setattr(speech_bubble_module, "PAGE_FADE_IN_MS", 1)
    bubble = PetSpeechBubble(style_id="classic_top")
    ...
    try:
        ...
        assert len(pages) > 1
        bubble._on_page_timeout()
        assert _wait_until(lambda: bubble._page_fade is None), "翻页动画未收尾"
        assert bubble._page_index == 1
        # 控制器每拍都会这样重发同一句歌词（内容、标题、排版参数全同）
        bubble.show_text(text, QRect(0, 0, 120, 120), 40000)
        assert bubble._page_index == 1, "同内容续期不该把翻页打回第一页"
        assert list(bubble._pages) == pages
        assert bubble.label.text() == pages[1], "续期后正文必须还是当前页"
    finally:
        bubble.dismiss()


def test_same_content_renewal_uses_effective_duration():
    """续期沿用"本次实际生效时长"：多页气泡被拉长后，续期不能按基准时长提前藏掉。"""
    _get_app()
    bubble = PetSpeechBubble(style_id="classic_top")
    text = "主人～DSH开始干活啦，人家会帮您盯着它的～" * 20
    bubble.show_text(text, QRect(0, 0, 120, 120), 40000)
    try:
        assert bubble._effective_duration_ms >= 40000, "多页气泡总时长应被拉长"
        bubble.show_text(text, QRect(0, 0, 120, 120), 1000)  # 续期给了更短的时长
        assert bubble._hide_timer.isActive()
        assert bubble._hide_timer.interval() >= 40000, "续期必须沿用生效时长"
    finally:
        bubble.dismiss()


def test_changed_content_still_rebuilds(monkeypatch):
    """反向守卫：内容变了必须走完整重建（否则换词不刷新）。"""
    _get_app()
    monkeypatch.setattr(speech_bubble_module, "PAGE_FADE_OUT_MS", 1)
    monkeypatch.setattr(speech_bubble_module, "PAGE_FADE_IN_MS", 1)
    bubble = PetSpeechBubble(style_id="classic_top")
    ...
    try:
        ...
        assert _wait_until(lambda: bubble._page_fade is None)
        assert bubble._page_index == 1
        bubble.show_text("换了一句完全不同的歌词～", QRect(0, 0, 120, 120), 40000)
        assert bubble._page_index == 0
        assert bubble.label.text().startswith("换了一句")
    finally:
        bubble.dismiss()


def test_locked_width_reuses_first_line_column():
    """锁宽：第二句起沿用第一句量出的列宽（否则气泡逐句变宽变窄、左右跳）。

    列宽函数被替换成"长句→300、短句→248"，因为离屏/无字体环境里真实度量会
    把所有文本都压到同一个下限，分不出锁宽到底有没有生效。
    """
    _get_app()
    bubble = PetSpeechBubble(style_id="classic_top")
    bubble._column_for_text = lambda text, anchor: 300 if len(text) > 5 else 248
    short = "短句"
    long_text = "这一句特别长特别长特别长特别长特别长特别长特别长特别长特别长哦"
    bubble.show_text(short, QRect(0, 0, 120, 120), 40000,
                     subtitle="歌名", title_first=True, width_locked=True)
    try:
        locked = bubble._locked_column
        assert locked == 248, "首句应量出并记住列宽基准"
        width = bubble.label.width()
        bubble.show_text(long_text, QRect(0, 0, 120, 120), 40000,
                         subtitle="歌名", title_first=True, width_locked=True)
        assert bubble._locked_column == locked, "锁宽后不该重算列宽"
        assert bubble.label.width() == width, "锁宽后长短句宽度必须一致"
    finally:
        bubble.dismiss()


def test_unlocked_lyric_bubble_recomputes_column():
    """未锁宽的普通气泡（非歌词）行为不变：列宽仍按内容算。"""
    _get_app()
    bubble = PetSpeechBubble(style_id="classic_top")
    bubble._column_for_text = lambda text, anchor: 300 if len(text) > 5 else 248
    try:
        bubble.show_text("短", QRect(0, 0, 120, 120), 4000, title_first=True)
        narrow = bubble.label.width()
        bubble.show_text("这一句特别长特别长特别长特别长", QRect(0, 0, 120, 120), 4000,
                         title_first=True, width_locked=False)
        assert bubble.label.width() > narrow, "没锁宽就该按当前句重算"
    finally:
        bubble.dismiss()


def test_empty_pages_falls_back_to_raw_text(monkeypatch):
    """分页后为空（如 "***" 这类规范化后没内容的行）：回退原始文本，正文不能留空。

    回归：正文空 + 标题在 ⇒ 用户看到的就是"歌词突然没了、只剩歌名"。
    """
    _get_app()
    bubble = PetSpeechBubble(style_id="classic_top")
    monkeypatch.setattr(speech_bubble_module, "paginate_bubble_text", lambda *a, **k: [])
    bubble.show_text("***", QRect(0, 0, 120, 120), 4000, subtitle="歌名", title_first=True)
    try:
        assert bubble.label.text() == "***", "规范化后为空时必须回退原始文本"
    finally:
        bubble.dismiss()
