...
def test_proxy_show_bubble_returns_window_result(tmp_path):
    """联动气泡的返回值必须透传：歌词控制器靠它判断"到底显示了没有"。

    代理层以前没有返回值（None），调用方会把"被提醒让路丢弃"当成"已显示"。
    """

    class _Win:
        def __init__(self, visible, result):
    ...
    hidden = _Win(False, True)
    refused = _Win(True, False)
    ok = _Win(True, True)

    # 可见窗明确拒绝（让路/被占用）→ 代理层返回 False
    proxy = MultiWindowProxy(_ProxyShell(Config(base=tmp_path), [hidden, refused]))
    assert proxy.show_bubble("歌词") is False
    assert refused.calls == ["歌词"]
    assert hidden.calls == [], "隐藏窗不该收到气泡"

    # 可见窗显示成功 → True
    proxy = MultiWindowProxy(_ProxyShell(Config(base=tmp_path), [ok]))
    assert proxy.show_bubble("歌词") is True

    # 一个可见窗都没有 → False（不能返回 None 让调用方误判）
    proxy = MultiWindowProxy(_ProxyShell(Config(base=tmp_path), [hidden]))
    assert proxy.show_bubble("歌词") is False
# ============================================================================
