        ...
        if self._preset.get("shape") == "breath_bubble" and not interactive:
            ...
            if self._title_first:
                ...
                self.label.setFixedSize(
                    # min_width=column：宽度恒为锁定的列宽。只把 min_width 抬到
                    # TITLE_FIRST_COLUMN 时，短句实际宽度仍是 min(column, 最宽+余量)
                    # ⇒ 长短句宽度不同、气泡照样左右跳。
                    bubble_label_size(
                        # text_slack 必须与上面的换行预算同源（含「气泡文字大小」系数）；
                        # min_width=column 再把 label 宽度钉死在本首歌锁定的列宽上
                        # ——只抬 min_width 到 TITLE_FIRST_COLUMN 时，短句实际宽度
                        # 仍是 min(column, 最宽+余量)，长短句照样左右跳。
